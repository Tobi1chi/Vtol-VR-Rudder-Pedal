...
void SerialCommand::begin(RudderSettings* settings) {
  ...
  Serial.println("INFO: ESP32-C3 USB HID is disabled. This build supports BLE mode only.");
}
...
void SerialCommand::cmdHelp(int argc, char** argv) {
  ...
  Serial.println("  mode hid   (disabled on ESP32-C3: USB Serial/JTAG is not USB HID)");
...
void SerialCommand::cmdStatus(int argc, char** argv) {
  ...
  Serial.println("STATUS: MODE=BLE");
  Serial.println("STATUS: HID=DISABLED (ESP32-C3 has USB Serial/JTAG, not native USB HID)");
...
void SerialCommand::cmdMode(int argc, char** argv) {
  ...
  if (strcmp(argv[1], "hid") == 0) {
    printErr("HID mode is disabled on ESP32-C3: the on-chip USB is USB Serial/JTAG only");
    return;
  }

  if (strcmp(argv[1], "ble") != 0) {
    printErr("mode must be ble; HID is disabled on ESP32-C3");
    return;
  }

  _settings->MODE = MODE_BLE;
  persistInt("MODE", MODE_BLE);
  printOk("MODE=BLE persisted");
}
...
bool SerialCommand::parseIntStrict(const char* text, int minValue, int maxValue, int& out) {
  ...
  if (errno != 0 || end == text || *end != '\0' || parsed < minValue || parsed > maxValue) {
    return false;
  }
...
bool SerialCommand::validateSettings(const RudderSettings& candidate, String& reason) {
  if (candidate.minRudder_L >= candidate.maxRudder_L) {
    reason = "min_l must be less than max_l";
    return false;
  }
  if (candidate.minRudder_R >= candidate.maxRudder_R) {
    reason = "min_r must be less than max_r";
    return false;
  }
  if ((candidate.minRudder_L + candidate.ERange) >= candidate.maxRudder_L) {
    reason = "left erange leaves no travel";
    return false;
  }
  if ((candidate.minRudder_R + candidate.ERange) >= candidate.maxRudder_R) {
    reason = "right erange leaves no travel";
    return false;
  }
  return true;
}
